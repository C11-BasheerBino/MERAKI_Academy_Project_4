...
const sendRequest = (e)=>{
  ...
  console.log(services.loggingId)

  axios.post("http://localhost:5000/requests",{
    providerId:providerId,
    serviceId:id,
    userId:services.loggingId,
      ...
      {(selectedFieldServices[0].services).map((element) => {
        return (
          <div style={{display:"flex",flexDirection:"column",gap:"20px",}}>
            ...
            <img src={element.image} alt={"picture of " + element.serviceName  } />
            <div>Description : {element.description}</div>
            <div>Price : {element.price} per hour</div>
            <button  id={element._id + ' ' +element.providerID} onClick={sendRequest}>Send Request</button>
          </div>
        );
      })}
    </div>
